...
6. 打开 **参数设置** 模块，并配置集群参数。
   ...
   | 模板 | 参数配置 | 说明 |
   |------|------|-------|
   | COMPLEX_OLTP 默认参数模板   | `enable_syslog_recycle:1` <br> `large_query_threshold:600s` <br> `max_syslog_file_count:300` <br> `_bloom_filter_ratio:35` <br> `_enable_defensive_check:0`  |  对应 **Complex OLTP** 负载类型，该模板仅适用于 OceanBase V4.3.0 及以上版本。   |
   ...
   | EXPRESS_OLAP 默认参数模板 | `enable_syslog_recycle:1` <br> `max_syslog_file_count:300` <br> `_bloom_filter_ratio:35` <br> `_enable_defensive_check:0`  | 对应 **Express OLAP** 负载类型，该模板仅适用于 OceanBase V4.3.0 及以上版本。 |
