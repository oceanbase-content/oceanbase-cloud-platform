...
6. 打开 **参数设置** 模块，并配置集群参数。
   ...
   | 模板 | 参数配置 | 说明 |
   |------|------|-------|
   | COMPLEX_OLTP 默认参数模板   | `enable_syslog_recycle:1` <br> `large_query_threshold:600s` <br> `max_syslog_file_count:300` <br> `_bloom_filter_ratio:35` <br> `_enable_defensive_check:0`  |  对应 **COMPLEX_OLTP** 负载类型，该模板仅适用于 OceanBase V4.3.0 及以上版本。   |
   | HTAP 默认参数模板   |  `enable_syslog_recycle:1` <br> `large_query_threshold:600s` <br> `max_syslog_file_count:300` <br> `_bloom_filter_ratio:3` |  对应 **HTAP** 负载类型，该模板仅适用于 OceanBase V4.3.0 及以上版本。 |
   | KV 默认参数模板   | `large_query_threshold:0ms` <br> `_bloom_filter_ratio:35`  |  对应 **OBKV** 负载类型，该模板仅适用于 OceanBase V4.3.0 及以上版本。   |
   | EXPRESS_OLAP 默认参数模板 | `enable_syslog_recycle:1` <br> `max_syslog_file_count:300` <br> `_bloom_filter_ratio:35` <br> `_enable_defensive_check:0`  | 对应 **EXPRESS_OLAP** 负载类型，该模板仅适用于 OceanBase V4.3.0 及以上版本。 |
   | OLAP 默认参数模板   | `enable_record_trace_log:false` <br> `large_query_threshold:0ms` <br> `trance_log_slow_query_watermark:7d` <br> `_bloom_filter_ratio:3`  |  对应 **OLAP** 负载类型，该模板仅适用于 OceanBase V4.3.0 及以上版本。   |
   | 2.2.77 默认参数模板   |  `autoinc_cache_refresh_interval:43200s` <br> `clog_sync_time_warn_threshold:1s` <br> `enable_merge_by_turn:FALSE` <br> `enable_one_phase_commit:FALSE` <br> `freeze_trigger_percentage:50` <br> `max_kept_major_version_number:1` <br> `minor_freeze_times:150` <br> `resource_soft_limit:100`   |  OceanBase 集群 V2.2.77 版本推荐使用的参数设置，供生产环境使用。   |
