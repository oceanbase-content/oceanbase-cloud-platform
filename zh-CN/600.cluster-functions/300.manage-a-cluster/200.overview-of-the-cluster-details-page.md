...
您可在集群概览页面查看集群基础信息，包括 **集群名、集群 ID、集群类型、OceanBase 版本号、Region 数量、部署模式、CPU 架构、Cgroup 配置、CPU 超卖额度、机器数量、负载类型、分区数、租户数量、创建者、OBProxy 集群、日志传输模式、同步状态、保护模式、软件安装路径、数据盘路径、日志盘路径**、**ConfigUrl** 和 **标签** 等信息。

![Image 102](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/ocp/433/%E9%9B%86%E7%BE%A4%E5%9F%BA%E6%9C%AC%E4%BF%A1%E6%81%AF.png)

|     参数     |  说明  |
|------------|--------|
| 集群名       | 集群的名称。单击右侧复制按钮，可对集群名称进行复制操作。 |
| 集群 ID       | 创建集群时自动生成的集群唯一标识 ID。 |
| 集群类型       | 分为 **主集群** 和 **备集群** 。<main id="notice" type='explain'><h4>说明</h4><p>当集群为 V4.0 及以上版本时，不再展示集群类型。</p></main>  |
...
| Cgroup 配置    | 显示当前集群是否开启 Cgroup。单击编辑按钮，可进行关闭或开启操作。<main id="notice" type='explain'><h4>说明</h4><p><ul><li>若您的业务场景为单租户、小规格（<14C）以及需要更高的数据库性能时，建议您关闭 Cgroup。</li><li>若主机上的操作系统内核版本低于 4.1.9，集群将无法创建租户，此时系统将默认关闭 Cgroup 且不显示 Cgroup 开关。</li><li>开启或关闭 Cgroup 将会重启集群，请谨慎操作。</li></ul></p></main>  |
...
| 保护模式       | 当集群类型为 **主集群** 时显示，详情参见 **保护模式**。<main id="notice" type='explain'><h4>说明</h4><p>当集群为 V4.0 及以上版本时，不再展示保护模式。</p></main>  |
| 软件安装路径     | 显示创建集群时填写的路径。|
| 数据盘路径      | 显示创建集群时填写的路径。 |
| 日志盘路径      | 显示创建集群时填写的路径。 |
...
您可在 Zone 列表中搜索并查看 **Zone 名**、**所属 Region**、**所在机房**、**机器数量**、**Root Service**、**CPU 架构** 和 **状态** 。并可在当前 Zone 中进行 **添加 OBServer**、**编辑**、**重启**、**停止** 和 **删除** 的操作。详情参见 [添加 OBServer 节点](../600.manage-an-observer/100.add-an-observer.md)、[编辑 Zone](../500.manage-a-zone/150.edit-a-zone.md)、[重启 Zone](../500.manage-a-zone/200.restart-a-zone.md)、[停止 Zone](../500.manage-a-zone/300.stop-a-zone.md)、[删除 Zone](../500.manage-a-zone/400.delete-a-zone.md)。
...
![09081044](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/ocp/430/observer%E5%88%97%E8%A1%A8.png)

## 查看仲裁服务列表

您可在仲裁服务列表中查看 **服务地址、状态、添加时间** 和 **启用服务的租户**，单击 **服务地址**，可查看仲裁服务管理页面。同时，您可在此页面进行 **添加仲裁服务、替换服务** 和 **移除服务** 操作，详情参见 [添加仲裁服务](../1350.manage-arbitration-services/800.add-arbitration-services.md)、[替换服务](../1350.manage-arbitration-services/900.replace-arbitration-services.md)、[移除服务](../1350.manage-arbitration-services/1000.remove-arbitration-services.md)。
