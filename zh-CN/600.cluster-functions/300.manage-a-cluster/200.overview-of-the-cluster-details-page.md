...
您可在集群概览页面查看集群基础信息，包括 **集群名、集群 ID、集群类型、OceanBase 版本号、CPU 超卖额度、Region 数量、部署模式、CPU 架构、机器数量、分区数、租户数量、创建者、OBProxy 集群、日志传输模式、同步状态、保护模式、软件安装路径、数据盘路径、日志盘路径**、**ConfigUrl** 和 **标签** 等信息。

![Image 102](https://obbusiness-private.oss-cn-shanghai.aliyuncs.com/doc/img/ocp/430/%E9%9B%86%E7%BE%A4%E5%9F%BA%E6%9C%AC%E4%BF%A1%E6%81%AF.png)
...
| CPU 超卖额度  | 配置 CPU 超卖额度。您可通过滑动块或输入框配置超卖比例，默认值为 120，取值范围为 101~200。<br><blockquote>**说明**</br>当集群为 V4.0 以下版本时，不支持配置集群 CPU 超卖额度。</blockquote>   |
...
| 机器数量       | 当前集群的主机数量。 |
| 分区数        | 当前集群的表的分区数。<br><blockquote>**说明**</br>当集群为 V4.0 及以上版本时，不再展示集群分区数。</blockquote> |
| 租户数量       | 当前集群的租户数量。 |
| 创建者        | 创建当前集群的用户名称。 |
