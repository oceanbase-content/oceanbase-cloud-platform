...
* 仅支持生成部分和记录变更有关的 Binlog event 而非所有的 Binlog event 都支持，即 Format_desc、Previous_gtids、Rotate、Gtid、Query、Table_map、Write_rows、Update_rows、Delete_rows 事件类型。
* 支持 GTID 模式。
...
您可参考如下流程使用 Binlog 服务：

1. 依据上文 **限制说明** 中的 **版本要求** ，创建满足条件的 OceanBase 集群、OceanBase 租户及 OBProxy 集群。
