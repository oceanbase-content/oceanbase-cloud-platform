...
5. 填写 **恢复备租户设置**，设置备份集信息。
   ...
   :::tab
   tab 页签1
   页签中第一行不可以 `tab、table` 等内容开始会被认为是新的页签，导致格式错误。
   tab 页签2
   可插入表格、段落、图片等内容
   tab 页签3
   可插入表格、段落、图片等内容
   ...
   | **备份集存储类型**        | 设置 **独立存储目录** 时展示，选择备份文件存储介质，取值范围：<ul><li> File：本地文件存储，一般为 NFS。   </li><li> OSS：阿里云 OSS 对象存储，仅 OceanBase V2.2.76 及以上版本支持。 </li><li>COS：腾讯云 COS 对象存储，仅 OceanBase V2.2.76 及以上、V4.0.0 以下版本或者 OceanBase V4.2.1 及以上版本支持。</li></ul> |
   | **数据备份目录**        | 设置 **独立存储目录** 时展示，填写备份文件在当前介质上的数据存储目录。 |
   | **日志备份目录**      | 设置 **独立存储目录** 时展示，填写备份文件在当前介质上的日志存储目录。 |
   | **访问域名**       | 填写备份集云存储端的域名。<ul><li>当 **备份集存储类型** 为 **OSS** 时展示，填写备份文件阿里云存储端的域名。支持在域名后添加端口号，如：`oss.ap-shanghai.myqcloud.com:8080`。</li><li>当 **备份集存储类型** 选择 **COS** 时展示，填写访问腾讯云 COS 存储桶中对象的域名。如：`cos.ap-beijing.myqcloud.com`。</li></ul> |
   | **访问用户**       | 设置 **独立存储目录** 且 **备份集存储类型** 为 **OSS** 时展示，填写访问阿里云存储端的用户。|
   | **访问密钥**       | 设置 **独立存储目录** 且 **备份集存储类型** 为 **OSS** 时展示，填写访问阿里云存储端的用户秘钥。 |
   | **备份目录**        | 未设置 **独立存储目录** 且 **备份集存储类型** 为 **File** 或 **COS** 时展示，填写备份文件在当前介质上的备份存储目录。 |
   | **存储目录**       | 未设置 **独立存储目录** 且 **备份集存储类型** 为 **OSS** 时展示，填写备份文件在当前介质上的存储目录。 |

   <div class="tabGroup tablist" id="tabgroup_1" data-bi-name="tab-group">
      <ul role="tablist" class='tablist'>
      <li role="presentation">
      <span role="tab" aria-controls="independent_storage_directory" data-tab="independent_storage_directory" tabindex="0" aria-selected="true" data-linktype="self-bookmark" data-bi-name="tab">设置<b>独立存储目录</b></span>
      </li>
      <li role="presentation"> 
      <span role="tab" aria-controls="non_independent_storage_directory" data-tab="non_tablist_PowerShell" tabindex="-1" data-linktype="self-bookmark" data-bi-name="tab" aria-selected="false">未设置<b>独立存储目录</b></span>
      </li>
      </ul>

      <section id="independent_storage_directory" role="tabpanel" data-tab="independent_storage_directory">
         <ul>
            <li>
               <h4>修改 OCP docker 容器文件</h4>
               <p>如果使用 OCP V2.3 及之前的版本，在安装集群前需要先修改 OCP docker 容器文件 <code>/home/admin/obztools/src/task/obauto_plus/ob_operate_plus.py</code>，在第 488 行的 <code>param</code> 变量值后追加 <code>"system_memory=30G"</code>。修改后的内容如下所示。</p>
                  <div class="codeHeader" id="code-try-2" data-bi-name="code-header"> </div>
                  <pre tabindex="0" class="has-inner-focus">
                  <code>param = param_must + ' -o "{0},__min_full_resource_pool_memory=1073741824,_ob_enable_prepared_statement=false,system_memory=30G"'.format(param_opt)</code>
                  </pre>
                     <main id="notice" type='alert'>
                     <h4>注意</h4>
                     <p>此处的 <code>system_memory</code> 的值可根据服务器实际内存进行调整，不建议调至更低，否则会引发一些系统内部错误。</p>
                     </main>
            </li>
            <li>
               <h4>OCP 页面修改参数</h4>
               <p><img src="https://help-static-aliyun-doc.aliyuncs.com/assets/img/zh-CN/3937199461/p429680.png" alt="1"></p>
            </li>
         </ul>
      </section>
      <section id="PowerShell" role="tabpanel" data-tab="PowerShell" hidden="hidden" aria-hidden="true">
   ...
   <div class="tabGroup tablist1" id="tabgroup_1" data-bi-name="tab-group">
      <ul role="tablist" class='tablist1'>
         <li role="presentation">
            <span role="tab" aria-controls="tabpanel_1_CLI" data-tab="tablist1_CLI1" tabindex="0" aria-selected="true" data-linktype="self-bookmark" data-bi-name="tab">通过 OCP 界面安装</span>
         </li>
         <li role="presentation">
            <span role="tab" aria-controls="tabpanel_1_PowerShell" data-tab="tablist1_PowerShell1" tabindex="-1" data-linktype="self-bookmark" data-bi-name="tab" aria-selected="false">通过命令行安装</span>
         </li>
      </ul>
      <section id="CLI1" role="tabpanel" data-tab="CLI1">
         <ul>
            <li>
               <h4>修改 OCP docker 容器文件</h4>
                  <p>如果使用 OCP V2.3 及之前的版本，在安装集群前需要先修改 OCP docker 容器文件 <code>/home/admin/obztools/src/task/obauto_plus/ob_operate_plus.py</code>，在第 488 行的 <code>param</code> 变量值后追加 <code>"system_memory=30G"</code>。修改后的内容如下所示。
                  </p>
                     <div class="codeHeader" id="code-try-2" data-bi-name="code-header"></div>
                     <pre tabindex="0" class="has-inner-focus">
                        <code>param = param_must + ' -o "{0},__min_full_resource_pool_memory=1073741824,_ob_enable_prepared_statement=false,system_memory=30G"'.format(param_opt)</code>
   ...
   <section id="PowerShell1" role="tabpanel" data-tab="PowerShell1" hidden="hidden" aria-hidden="true">
      <main id="notice" type='explain'>
         <h4>注意</h4>
      </main>
   </section>
