...
查看租户基本信息，包括 **所属集群、OceanBase 版本号、租户模式、字符集、Collation、部署分布、OBProxy/连接串、服务名、标签、锁定状态、创建时间、备注** 和 **仲裁服务** 等信息。
...
* 单击连接串后的图标，可直接进行复制。
  ...
  * 租户所属的集群可通过直连方式关联到 OBProxy，示例如下：
    ...
    ```
* 单击 **添加服务名**，可直接添加服务名。服务名以英文字母或者下划线开头，可包含英文、数字和下划线，且长度为 2\~64 个字符。您也可以通过 **操作** 下拉菜单，编辑或删除已添加的服务名。此外，对于主备租户，系统支持 **同步变更主备租户服务名**。
