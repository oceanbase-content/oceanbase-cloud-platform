...
7. 打开 **自定义设置**，支持您执行集群级别用户配置（如操作系统属主用户）、路径设置（如软件安装路径、数据盘路径、日志盘路径）以及端口设置（如 SQL 端口、RPC 端口）。
   ...
   * 配置相关路径：
      ...
      | **日志盘路径**   | 默认为 `/data/log1`，支持自定义路径。生产环境下，OceanBase 数据库建议日志盘空间为主机内存空间的三倍以及以上；同时，为了避免出现性能问题，不建议数据目录和日志目录挂载在同一块磁盘上。 <main id="notice" type='explain'><h4>说明</h4>OceanBase 数据库从 V4.2.1 BP5 开始（不包括 V4.2.2、V4.2.3），支持将 slog 从数据（data）盘中独立出来，即 slog 与数据文件无需在同一个磁盘上。您可配置所使用的磁盘，将 slog 与 clog 共用一块 SSD 固态硬盘。当 slog 与 clog 共用一块盘时，若配置 <code>log_disk_size=0</code>，会被视为 clog 独占，即默认 <code>log_disk_percentage</code> 为 90%。如果 slog 与 clog 所在磁盘的大小小于 40GiB，此时为 slog 预留的空间则不足 4GiB，可能会影响 slog 的写入。更多 slog 相关说明，可参考 <a href="https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000819002">OBServer 节点安装目录结构</a>。</main>     |
