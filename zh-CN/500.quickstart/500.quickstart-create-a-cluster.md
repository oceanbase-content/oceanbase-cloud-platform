...
4. 设置集群的部署模式信息。
   ...
   | **添加仲裁服务** |  选择是否为集群添加仲裁服务。您可在服务下拉列表中选择需要添加的服务，或单击 **添加服务** 为集群新建仲裁服务，详情参见[添加仲裁服务](../600.cluster-functions/400.manage-arbitration-services/800.add-arbitration-services.md)。  <blockquote>**说明**</br>仅支持为 V4.1.0 及以上版本的主集群添加仲裁服务。</blockquote>  |
...
6. 打开 **参数设置** 模块，并配置集群参数。
    ...
    * 也可通过如图 ② 处，单击 **选择参数模板** ，然后选择一个参数模板，系统会将模板中的参数连同配置自动填充到此处。在未创建集群参数模板的情况下，您可单击 **新建集群参数模板**，为集群创建参数模板，详情可参考 [集群参数模板管理](../600.cluster-functions/1200.cluster-parameters-template-management.md)。
...
7. 打开 **自定义设置**，支持您执行集群级别用户配置（如操作系统属主用户）、路径设置（如软件安装路径、数据盘路径、日志盘路径）以及端口设置（如 SQL 端口、RPC 端口）。
   ...
   * 配置操作系统所属主用户：
      ...
      该用户为配置安装、运行 OBServer 的操作系统用户，不支持编辑。您可通过调整 `ocp.operation.default.os.user` 参数的默认配置来修改此用户，该参数的修改仅在创建分布式集群、[创建 OBProxy 集群](../800.obproxy-functions/200.create-an-obproxy-cluster.md) 和 [创建仲裁服务](../600.cluster-functions/400.manage-arbitration-services/200.create-arbitration-services.md) 时产生影响，不影响已有集群的其他配置。
   ...
   * 配置相关路径：
      ...
      | **日志盘路径**   | 默认为 `/data/log1`，支持自定义路径。生产环境下，OceanBase 数据库建议日志盘空间为主机内存空间的三倍以及以上；同时，为了避免出现性能问题，不建议数据目录和日志目录挂载在同一块磁盘上。 <main id="notice" type='explain'><h4>说明</h4><ul><li>OceanBase 数据库从 V4.2.1 BP5 开始（不包括 V4.2.2、V4.2.3），支持将 slog 从数据（data）盘中独立出来，即 slog 与数据文件无需在同一个磁盘上。您可配置所使用的磁盘，将 slog 与 clog 共用一块 SSD 固态硬盘。</li><li>当 slog 与 clog 共用一块盘时，若配置 <code>log_disk_size=0</code>，会被视为 clog 独占，即默认 <code>log_disk_percentage</code> 为 90%。如果 slog 与 clog 所在磁盘的大小小于 40GiB，此时为 slog 预留的空间则不足 4GiB，可能会影响 slog 的写入。</li>更多 slog 相关说明，可参考 <a href="https://www.oceanbase.com/docs/common-oceanbase-database-cn-1000000000819002">OBServer 节点安装目录结构</a>。</ul></main>     |
