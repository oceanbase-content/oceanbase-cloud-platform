...
4. 填写 **基本设置** 。需要完成的基本设置及其相关说明如下表所示。
   ...
   | **服务名** | 可选。服务名是 OceanBase 提供的一种新的数据库访问方式，可以视其为集群名和租户名的别名，实现应用程序通过服务名的方式访问数据库。关于服务名的更多介绍，可参见 [管理服务名](../700.tenant-functions/600.manage-a-tenant/800.manage-service-name.md)。<ul><li>服务名以英文字母开头，可包含英文、数字和下划线，且长度为 2\~64 个字符。</li><li>需同时满足如下版本要求，方可为租户配置服务名：<ul><li>OceanBase：V4.2.19、[V4.2.4, V4.3.0)、[V4.3.3, +∞)。</li><li>OBProxy：[V4.3.1.0, +∞)。</li></ul></li> |
   | **备注**        | 可选，备注说明信息。 |
