...
1. 获取 AK/SK。
    ...
    前往您的 OCP 管控台，单击右上角 **个人设置** ，选择 **一键创建AccessKey**。

2. 创建要签名的字符串。

    根据请求信息和额外信息（如算法、请求时间等）创建代签字符串。

3. 计算签名。

    使用签名秘钥（Access Key Secret）对待签字符串执行加密哈希操作来计算签名。使用签名密钥作为此操作的哈希密钥。
...
<h4>注意</h4>
<p>uri 与 query 参数拼接是通过 “+” 而非 “\n”。另外某项内容可能为空（例如: Md5Payload），当为空时需注意连接符 “\n” 是存在的。</p>
</main>

从公式可以看出 message 由 **Http Method**、**Md5Payload**、**ContentType**、**RequestDateTime**、**Host**、**OCP-Headers** 和 **PathAndQueryParams** 参数七部分组成，它们的具体含义及获取方式如下：

* **Http Method**：HTTP 请求方法，如 GET、POST、PUT、DELETE 等。
...
public class OcpSignDemo {
    ...
    public static void main(String[] args) throws Exception {
        ...
        params.put("labels", "svr_ip:xxx.xxx.xxx.xxx");
        ...
        String sign = getSign("xxx.xxx.xxx.xxx:8080", path, "GET", headers, params, null, accessKeySecret, rfcDate);
