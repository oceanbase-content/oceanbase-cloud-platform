...
|  参数   |  类型   | 必选 |          描述          |
|-------|-------|---------|----------------------|
| hosts | Array | 是  | 待接管的 OBProxy 所在主机信息。 |

**ObproxyTakeOverParam 数据结构**

|   参数    |   类型    | 必选 | 示例值  |           描述           |
|---------|---------|--------|--------|------------------------|
| hostId  | Long    | 是  | 1    | 待接管的 OBProxy 所在主机 ID。  |
| sqlPort | Integer | 是  | 2881 | 待接管的 OBProxy 的 SQL 端口。 |
| proxysysPasswd  | String    | 否  | xxxx    | 主机上部署的 obproxy 进程的 root@proxysys 密码。    |
