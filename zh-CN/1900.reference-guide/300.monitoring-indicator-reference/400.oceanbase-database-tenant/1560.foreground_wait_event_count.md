...
本节为您介绍 OceanBase 租户的监控指标前台等待事件数，该指标细分为 11 个指标，分别为 on_cpu（处于 on cpu 状态执行数据库命令的会话数）、application（处于客户端代码导致的等待事件数）、configuration（处于数据库或实例资源配置不足而导致的等待事件数）、administrative（处于数据库管理员输入命令导致用户等待的等待事件数）、concurrency（处于等待数据库内部资源的等待事件数）、commit（处于在日志提交有关的等待事件数）、network（处于网络通信相关的等待事件数）、user_io（处于用户 I/O 的等待事件数）、system_io（处于前台进程 I/O 的等待事件数）、cluster（处于与集群相关的等待事件数）、other（处于不属于其他类型的等待事件数）。您可指定统计范围，查询租户在单个 Zone 或 OBServer 节点的 **前台等待事件数** 数据。
