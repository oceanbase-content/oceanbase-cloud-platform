...
| DB Time       | db_cpu  | 租户数据库前台工作时间（不包含非空闲等待，如等锁）   | sum(rate(ob_sysstat{stat_id="200002",@LABELS}[@INTERVAL])) by (@GBLABELS) |
| DB Time       | non_idle_wait_time  | 租户数据库前台非空闲等待时间   | sum(rate(ob_sysstat{stat_id="200010",@LABELS}[@INTERVAL])) by (@GBLABELS) |
| DB Time       | background_db_cpu  | 租户数据库后台工作时间（不包含非空闲等待，如等锁）   | sum(rate(ob_sysstat{stat_id="200006",@LABELS}[@INTERVAL])) by (@GBLABELS) |
| DB Time       | background_non_idle_wait_time  | 租户数据库后台非空闲等待时间   | sum(rate(ob_sysstat{stat_id="200013",@LABELS}[@INTERVAL])) by (@GBLABELS) |
| SQL 执行事件数       | sql_event_in_parse  | 租户处于 parse 状态的执行事件数   | sum(ob_sql_event_in_parse{@LABELS}) by (@GBLABELS) |
| SQL 执行事件数       | sql_event_in_pl_parse  | 租户处于 pl parse 状态的执行事件数   | sum(ob_sql_event_in_pl_parse{@LABELS}) by (@GBLABELS) |
| SQL 执行事件数      | sql_event_in_plan_cache  | 租户处于 plan cache 状态的执行事件数   | sum(ob_sql_event_in_plan_cache{@LABELS}) by (@GBLABELS) |
| SQL 执行事件数       | sql_event_in_sql_optimize  | 租户处于 sql optimize 状态的执行事件数   | sum(ob_sql_event_in_sql_optimize{@LABELS}) by (@GBLABELS) |
| SQL 执行事件数       | sql_event_in_sql_execution  | 租户处于 sql execution 状态的执行事件数   | sum(ob_sql_event_in_sql_execution{@LABELS}) by (@GBLABELS) |
| SQL 执行事件数       | sql_event_in_px_execution  | 租户处于 px execution 状态的执行事件数   | sum(ob_sql_event_in_px_execution{@LABELS}) by (@GBLABELS) |
| SQL 执行事件数       | sql_event_in_sequence_load  | 租户处于 sequence load 状态的执行事件数   | sum(ob_sql_event_in_sequence_load{@LABELS}) by (@GBLABELS) |
| SQL 执行事件数       | sql_event_in_committing  | 租户处于 committing 状态的执行事件数   | sum(ob_sql_event_in_committing{@LABELS}) by (@GBLABELS) |
| SQL 执行事件数       | sql_event_in_storage_read  | 租户处于 storage read 状态的执行事件数   | sum(ob_sql_event_in_storage_read{@LABELS}) by (@GBLABELS) |
| SQL 执行事件数       | sql_event_in_storage_write  | 租户处于 storage write 状态的执行事件数   | sum(ob_sql_event_in_storage_write{@LABELS}) by (@GBLABELS) |
| SQL 执行事件数       | sql_event_in_remote_das_execution  | 租户处于 remote das execution 状态的执行事件数   | sum(ob_sql_event_in_remote_das_execution{@LABELS}) by (@GBLABELS) |
| SQL 执行事件数       | sql_event_in_plsql_compilation  | 租户处于 plsql compilation 状态的执行事件数   | sum(ob_sql_event_in_plsql_compilation{@LABELS}) by (@GBLABELS) |
| SQL 执行事件数       | sql_event_in_plsql_execution  | 租户处于 plsql execution 状态的执行事件数   | sum(ob_sql_event_in_plsql_execution{@LABELS}) by (@GBLABELS) |
| 前台等待事件数       | ob_foreground_on_cpu_waitevent_cnt  | 租户前台处于 on_cpu 状态的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="ON_CPU",@LABELS}) by (@GBLABELS) |
| 前台等待事件数       | ob_foreground_application_waitevent_cnt  | 租户前台处于 application 状态的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="APPLICATION",@LABELS}) by (@GBLABELS) |
| 前台等待事件数       | ob_foreground_configuration_waitevent_cnt  | 租户前台处于 configuration 状态的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="CONFIGURATION",@LABELS}) by (@GBLABELS) |
| 前台等待事件数       | ob_foreground_administrative_waitevent_cnt  | 租户前台处于 administrative 状态的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="ADMINISTRATIVE",@LABELS}) by (@GBLABELS) |
| 前台等待事件数       | ob_foreground_concurrency_waitevent_cnt  | 租户前台处于 concurrency 状态的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="CONCURRENCY",@LABELS}) by (@GBLABELS)  |
| 前台等待事件数       | ob_foreground_commit_waitevent_cnt  | 租户前台处于 commit 状态的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="COMMIT",@LABELS}) by (@GBLABELS) |
| 前台等待事件数       | ob_foreground_network_waitevent_cnt  | 租户前台处于 network 状态的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="NETWORK",@LABELS}) by (@GBLABELS) |
| 前台等待事件数       | ob_foreground_user_io_waitevent_cnt | 租户前台处于 user_io 状态的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="USER_IO",@LABELS}) by (@GBLABELS)  |
| 前台等待事件数       | ob_foreground_system_io_waitevent_cnt  | 租户前台处于 system_io 状态的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="SYSTEM_IO",@LABELS}) by (@GBLABELS) |
| 前台等待事件数       | ob_foreground_cluster_waitevent_cnt  | 租户前台处于 cluster 状态的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="CLUSTER",@LABELS}) by (@GBLABELS)   |
| 前台等待事件数       | ob_foreground_other_waitevent_cnt  | 租户前台处于 other 状态的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="OTHER",@LABELS}) by (@GBLABELS)   |
| 后台等待事件数       | ob_background_on_cpu_waitevent_cnt  | 租户后台处于 on_cpu 状态的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="ON_CPU",@LABELS}) by (@GBLABELS) |
| 后台等待事件数       | ob_background_application_waitevent_cnt  | 租户后台处于 application 状态的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="APPLICATION",@LABELS}) by (@GBLABELS)   |
| 后台等待事件数       | ob_background_configuration_waitevent_cnt  | 租户后台处于 configuration 状态的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="CONFIGURATION",@LABELS}) by (@GBLABELS)   |
| 后台等待事件数       | ob_background_administrative_waitevent_cnt  | 租户后台处于 administrative 状态的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="ADMINISTRATIVE",@LABELS}) by (@GBLABELS) |
| 后台等待事件数       | ob_background_concurrency_waitevent_cnt  | 租户后台处于 concurrency 状态的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="CONCURRENCY",@LABELS}) by (@GBLABELS)   |
| 后台等待事件数       | ob_background_commit_waitevent_cnt | 租户后台处于 commit 状态的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="COMMIT",@LABELS}) by (@GBLABELS) |
| 后台等待事件数       | ob_background_network_waitevent_cnt  | 租户后台处于 network 状态的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="NETWORK",@LABELS}) by (@GBLABELS)   |
| 后台等待事件数       | ob_background_user_io_waitevent_cnt  | 租户后台处于 user_io 状态的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="USER_IO",@LABELS}) by (@GBLABELS)   |
| 后台等待事件数       | ob_background_system_io_waitevent_cnt  | 租户后台处于 system_io 状态的等待事件数    | sum(ob_background_waitevent_cnt{wait_class="SYSTEM_IO",@LABELS}) by (@GBLABELS)   |
| 后台等待事件数       | ob_background_cluster_waitevent_cnt  | 租户后台处于 cluster 状态的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="CLUSTER",@LABELS}) by (@GBLABELS)   |
| 后台等待事件数       | ob_background_other_waitevent_cnt | 租户后台处于 other 状态的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="OTHER",@LABELS}) by (@GBLABELS)   |
