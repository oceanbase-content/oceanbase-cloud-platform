...
| SQL 执行阶段       | sql_event_in_plsql_execution  | 正在 PL 编译的活跃会话数   | sum(ob_sql_event_in_plsql_execution{@LABELS}) by (@GBLABELS) |
...
| 前台工作负载       | ob_foreground_on_cpu_waitevent_cnt  | 处于 on cpu 状态执行数据库命令的会话数   | sum(ob_foreground_waitevent_cnt{wait_class="ON_CPU",@LABELS}) by (@GBLABELS) |
| 前台工作负载       | ob_foreground_application_waitevent_cnt  | 处于客户端代码导致的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="APPLICATION",@LABELS}) by (@GBLABELS) |
| 前台工作负载       | ob_foreground_configuration_waitevent_cnt  | 处于数据库或实例资源配置不足而导致的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="CONFIGURATION",@LABELS}) by (@GBLABELS) |
| 前台工作负载       | ob_foreground_administrative_waitevent_cnt  | 处于数据库管理员输入命令导致用户等待的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="ADMINISTRATIVE",@LABELS}) by (@GBLABELS) |
| 前台工作负载       | ob_foreground_concurrency_waitevent_cnt  | 处于等待数据库内部资源的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="CONCURRENCY",@LABELS}) by (@GBLABELS)  |
| 前台工作负载       | ob_foreground_commit_waitevent_cnt  | 处于在日志提交有关的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="COMMIT",@LABELS}) by (@GBLABELS) |
| 前台工作负载       | ob_foreground_network_waitevent_cnt  | 处于网络通信相关的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="NETWORK",@LABELS}) by (@GBLABELS) |
| 前台工作负载       | ob_foreground_user_io_waitevent_cnt | 处于用户 I/O 的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="USER_IO",@LABELS}) by (@GBLABELS)  |
| 前台工作负载       | ob_foreground_system_io_waitevent_cnt  | 处于前台进程 I/O 的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="SYSTEM_IO",@LABELS}) by (@GBLABELS) |
| 前台工作负载       | ob_foreground_cluster_waitevent_cnt  | 处于与集群相关的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="CLUSTER",@LABELS}) by (@GBLABELS)   |
| 前台工作负载       | ob_foreground_other_waitevent_cnt  | 处于不属于其他类型的等待事件数   | sum(ob_foreground_waitevent_cnt{wait_class="OTHER",@LABELS}) by (@GBLABELS)   |
| 后台工作负载       | ob_background_on_cpu_waitevent_cnt  | 处于 on cpu 状态执行数据库命令的会话数   | sum(ob_background_waitevent_cnt{wait_class="ON_CPU",@LABELS}) by (@GBLABELS) |
| 后台工作负载       | ob_background_application_waitevent_cnt  | 处于客户端代码导致的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="APPLICATION",@LABELS}) by (@GBLABELS)   |
| 后台工作负载       | ob_background_configuration_waitevent_cnt  | 处于数据库或实例资源配置不足而导致的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="CONFIGURATION",@LABELS}) by (@GBLABELS)   |
| 后台工作负载       | ob_background_administrative_waitevent_cnt  | 处于数据库管理员输入命令导致用户等待的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="ADMINISTRATIVE",@LABELS}) by (@GBLABELS) |
| 后台工作负载       | ob_background_concurrency_waitevent_cnt  | 处于等待数据库内部资源的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="CONCURRENCY",@LABELS}) by (@GBLABELS)   |
| 后台工作负载       | ob_background_commit_waitevent_cnt | 处于在日志提交有关的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="COMMIT",@LABELS}) by (@GBLABELS) |
| 后台工作负载       | ob_background_network_waitevent_cnt  | 处于网络通信相关的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="NETWORK",@LABELS}) by (@GBLABELS)   |
| 后台工作负载       | ob_background_user_io_waitevent_cnt  | 处于用户 I/O 的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="USER_IO",@LABELS}) by (@GBLABELS)   |
| 后台工作负载       | ob_background_system_io_waitevent_cnt  | 处于后台进程 I/O 的等待事件数    | sum(ob_background_waitevent_cnt{wait_class="SYSTEM_IO",@LABELS}) by (@GBLABELS)   |
| 后台工作负载       | ob_background_cluster_waitevent_cnt  | 处于与集群相关的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="CLUSTER",@LABELS}) by (@GBLABELS)   |
| 后台工作负载       | ob_background_other_waitevent_cnt | 处于不属于其他类型的等待事件数   | sum(ob_background_waitevent_cnt{wait_class="OTHER",@LABELS}) by (@GBLABELS)   |
